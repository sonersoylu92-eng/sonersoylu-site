...
export const SPEC = {
  hubHeight: 80,          // m — Aliağa/Bergama'da yaygın varyant
  rotorDiameter: 90,
  bladeLength: 43.8,
  hubDiameter: 3.2,
  towerBase: 4.30,        // m dış çap
  towerTop: 2.90,
  nacelleLen: 10.4,
  nacelleWid: 3.40,
  nacelleHei: 3.60,
...
const TOWER_SEGMENTS = (() => {
  const H = SPEC.hubHeight - 2.2;           // nacelle yaw düzlemi
  const n = 4;
  const segs = [];
  let y = 0;
  for (let i = 0; i < n; i++) {
...
const STATIONS = [
  { r: 0.000, c: 2.20, tw: 13.0, tc: 1.00, cyl: 1.00 },
  { r: 0.035, c: 2.20, tw: 13.0, tc: 1.00, cyl: 1.00 },
  { r: 0.070, c: 2.55, tw: 13.0, tc: 0.72, cyl: 0.72 },
  { r: 0.120, c: 3.20, tw: 12.2, tc: 0.48, cyl: 0.34 },
  { r: 0.180, c: 3.48, tw: 10.4, tc: 0.36, cyl: 0.10 },
  { r: 0.250, c: 3.30, tw:  8.2, tc: 0.29, cyl: 0.00 },
  { r: 0.350, c: 3.05, tw:  5.8, tc: 0.25, cyl: 0.00 },
  { r: 0.450, c: 2.78, tw:  4.2, tc: 0.22, cyl: 0.00 },
  { r: 0.550, c: 2.48, tw:  3.0, tc: 0.21, cyl: 0.00 },
  { r: 0.650, c: 2.16, tw:  2.1, tc: 0.20, cyl: 0.00 },
  { r: 0.750, c: 1.84, tw:  1.4, tc: 0.19, cyl: 0.00 },
  { r: 0.850, c: 1.52, tw:  0.9, tc: 0.18, cyl: 0.00 },
  { r: 0.930, c: 1.20, tw:  0.5, tc: 0.18, cyl: 0.00 },
  { r: 0.960, c: 1.02, tw:  0.2, tc: 0.17, cyl: 0.00 },
  { r: 0.990, c: 0.74, tw:  0.1, tc: 0.18, cyl: 0.00 },
  { r: 1.000, c: 0.40, tw:  0.0, tc: 0.22, cyl: 0.00 },
];
...
export function createScene(canvas) {
  ...
  [[-338, 286, 0.9], [-536, 364, 1.0], [292, 408, 0.95], [498, 276, 1.05], [-106, 482, 1.0], [126, 570, 0.95]]
    .forEach(([x, z, sc], i) => {
...
export const STAGES = [
  {
    id: 'temel',
    title: 'Temel ve ankraj kafesi',
    lead: 'Kazı, ankraj kafesinin terazilenmesi ve tek seferde beton dökümü. Kafesin düzlemselliği burada tutmazsa kule bütün ömrü boyunca o hatayı taşır.',
    facts: [['Beton hacmi', '≈ 450–600 m³'], ['Donatı', '≈ 45–60 t'], ['Kür süresi', '21–28 gün'], ['Ankraj cıvatası', '100–140 adet, M42–M48']],
    note: 'Ankraj cıvatalarının ön germesi kademeli ve çapraz sırayla yapılır; ilk dolu yük öncesi tork/germe kontrolü tekrarlanır.',
    cam: { pos: [26, 19, -31], target: [0, 1.5, 0] },
  },
  {
    id: 'kule-alt',
    title: 'Kule alt bölümü',
    lead: 'İlk iki segment ankraj flanşına oturuyor. Kapı, iç merdiven ve kablo askıları bu bölümde; alt segment aynı zamanda trafo bağlantısının geldiği yer.',
    facts: [['Segment sayısı', '4 (80 m göbek)'], ['Taban çapı', '4,30 m'], ['Malzeme', 'S355 çelik'], ['Kule ağırlığı', '≈ 200–319 t']],
    note: 'Flanş yüzeyleri kaldırma öncesi temizlenir. Cıvatalar önce el sıkılığında, sonra hidrolik germe ile üç turda son değerine getirilir.',
    cam: { pos: [56, 34, -70], target: [0, 19, 0] },
  },
  {
    id: 'kule-ust',
    title: 'Kule üst bölümü',
    lead: 'Üst iki segment ile göbek yüksekliği 80 metreye çıkıyor. Her segment arası flanş bağlantısı ayrı bir germe operasyonu.',
    facts: [['Göbek yüksekliği', '80 m (70–120 m seçenekli)'], ['Tepe çapı', '2,90 m'], ['İç donanım', 'Merdiven, platform, servis asansörü'], ['Rüzgâr limiti', 'Kaldırmada ≈ 8–10 m/s']],
    note: 'Kule tamamlandıktan sonra düşeylik ölçümü yapılır; sapma tolerans dışıysa nacelle kaldırılmadan düzeltilir.',
    cam: { pos: [72, 52, -96], target: [0, 42, 0] },
  },
  {
    id: 'nacelle',
    title: 'Nacelle kaldırma',
    lead: '91 tonluk makine dairesi tek parça olarak yaw yatağına oturuyor. Günün en kritik kaldırması: yükseklik, ağırlık ve rüzgâr aynı anda sınırda.',
    facts: [['Nacelle ağırlığı', '≈ 91 t'], ['Gövde', 'Küresel grafitli dökme demir'], ['Kapak', 'Cam elyaf takviyeli plastik'], ['Yaw tahriki', '2 asenkron motor, ≈ 0,5°/s']],
    note: 'Yaw yatağı cıvataları oturduktan sonra makine dairesi geçici olarak kilitlenir; kablo askıları takılmadan yaw serbest bırakılmaz.',
    cam: { pos: [34, 87, -46], target: [0, 74, -2] },
  },
  {
    id: 'gobek',
    title: 'Göbek ve pitch sistemi',
    lead: 'Hub, pitch tahrikleri ve acil durum enerji depolamasıyla birlikte ana şafta bağlanıyor. Bu aşamada nacelle içindeki güç aktarma organları da devrede.',
    facts: [['Hub + pitch', '≈ 25 t'], ['Ana şaft', '42CrMo4 / 34CrNiMo6'], ['Ana yatak', 'Kendinden hizalamalı makaralı'], ['Dişli kutusu', '3 planet + 1 alın, i ≈ 1:77'], ['Jeneratör', 'Çift beslemeli asenkron, 660 V']],
    note: 'Pitch kutularının besleme ve haberleşme bağlantıları hub içinde slip ring üzerinden geçer; her kanat bağımsız çalışır, bu aynı zamanda aerodinamik frendir.',
    cam: { pos: [27, 83, -21], target: [0, 79, -2] },
  },
  {
    id: 'kanatlar',
    title: 'Kanat montajı',
    lead: 'Üç kanat, 43,8 metre ve her biri ~10 tonluk cam elyaf yapı. Tekli kanat montajında kanat yatay askıda hub yatağına getirilip cıvatalanır.',
    facts: [['Kanat uzunluğu', '43,8 m'], ['Kanat ağırlığı', '≈ 10,2 t'], ['Malzeme', 'Cam elyaf takviyeli plastik'], ['Rotor çapı', '90 m'], ['Süpürme alanı', '6.362 m²']],
    note: 'Kök cıvataları (T-bolt) çapraz sırayla ve kademeli torkla sıkılır. Kanat yüzeyinde nakliye izi, jelkot çatlağı ve şimşek iletim noktaları montaj öncesi kontrol edilir.',
    cam: { pos: [106, 88, -144], target: [0, 74, -4] },
  },
  {
    id: 'devreye',
    title: 'Devreye alma',
    lead: 'Kablolama, topraklama, yağ ve fren testleri, yaw ve pitch fonksiyon kontrolleri. Ardından ilk şebeke senkronizasyonu ve yük testleri.',
    facts: [['Nominal güç', '2.500 kW'], ['Devreye giriş rüzgârı', '3,0 m/s'], ['Nominal rüzgâr', '13,5 m/s'], ['Kesme rüzgârı', '25,0 m/s'], ['Rotor devri', 'maks. 18,1 d/dk'], ['Kanat ucu hızı', '≈ 85 m/s']],
    note: 'İlk 500 saatlik dönemde tork kontrolü, yağ analizi ve titreşim ölçümü tekrarlanır. Devreye alma raporu türbinin ömür boyu referansıdır.',
    cam: { pos: [126, 82, -180], target: [0, 60, 0] },
  },
];
